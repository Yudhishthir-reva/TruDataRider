        ...
        components
            ...
            .map { group in
                DashboardSection(
                    ...
                    items: group.subMenu.flatMap(\.componentData.items).sorted { $0.orderInt < $1.orderInt }
                )
            }
...
enum RiderOperationsType: String, Hashable {
    ...
    var title: String {
        ...
        case .actions: return "My Actions"
        }
    }

    var routes: Set<String> {
        switch self {
        case .activity: return ["today_achievements"]
        case .seller: return ["registered_sellers"]
        case .actions: return ["attendance", "regularization_requests", "apply_reimbursements", "view_leaves"]
        }
    }
}
